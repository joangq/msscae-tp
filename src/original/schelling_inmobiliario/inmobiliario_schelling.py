...
def precio_propiedad(C_propio, C_distinto):
...
def utilidad(K, precio, alpha=0.5):
...
class mercado_inmobiliario:
    
    def __init__(self, L, configuracion=None, alpha=0.5):
        """
        Inicializa una instancia de la clase Schelling.

        Parámetros:
            L (int): Tamaño de la cuadrícula (lattice).
            configuracion (array_like, opcional): Configuración inicial de la cuadrícula. Si no se proporciona,
                se generará una configuración aleatoria.
        """
        self.L = L
        self.alpha = alpha
        ...
        self.K = np.ones((self.L, self.L))  # Inicialización del capital de cada individuo K=1

        # Calcular la utilidad inicial para cada agente en la configuración
        self.U = np.zeros((self.L, self.L))  # Matriz de ceros a ser rellenada
    ...
    def utilidad_media(self):
        
        return np.mean(self.U.flatten())
    
    def capital_medio(self):
    ...
    def _num_vecinos(self, i_a, j_a, i, j):
        """
    ...
    def proponer_intercambio(self):
        ...
        i1, j1, i2, j2 = np.random.randint(0, self.L, size=4)
    ...
    def ronda_intercambio(self):
        ...
        for step in range(N_intercambios):
